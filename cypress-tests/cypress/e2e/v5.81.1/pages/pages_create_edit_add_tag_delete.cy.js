...
context('Error en inicio de sesion por email y contraseña incorrecta.', () => {
  ...
  cy.screenshot('../screenshots/create_edit_add_tag_delete/before_4');
    cy.wait(500);

  // La URL contiene la ruta de pages.
  cy.url().should('contain','/pages');
  cy.screenshot('../screenshots/create_edit_add_tag_delete/before_5');
    cy.wait(500);

   // Seleccioanr el boton de pages.
   cy.get('a[href="#/editor/page/"]').click();
   cy.screenshot('../screenshots/create_edit_add_tag_delete/before_6');
    cy.wait(500);

   // La URL contiene la ruta de pages.
  cy.url().should('contain','/editor/page');
  cy.screenshot('../screenshots/create_edit_add_tag_delete/before_7');
    cy.wait(500);
...
cy.screenshot('../screenshots/create_edit_add_tag_delete/before_9');
    cy.wait(500);

   cy.get('button[title="Settings"]').click();
cy.screenshot('../screenshots/create_edit_add_tag_delete/before_10');
    cy.wait(500);

   cy.get('input[id="url"]').type('example-cypress');
cy.screenshot('../screenshots/create_edit_add_tag_delete/before_11');
    cy.wait(500);

   cy.get('button[title="Settings"]').click();
cy.screenshot('../screenshots/create_edit_add_tag_delete/before_12');
    cy.wait(500);

   cy.get('button.gh-btn.gh-btn-editor.darkgrey.gh-publish-trigger[data-test-button="publish-flow"]')
   .contains('Publish')
   .click();
   cy.screenshot('../screenshots/create_edit_add_tag_delete/before_13');
    cy.wait(500);

   cy.get('button[data-test-button="continue"]').click();
   cy.screenshot('../screenshots/create_edit_add_tag_delete/before_14');
   cy.wait(500);

   cy.get('button[data-test-button="confirm-publish"]').click();
   cy.screenshot('../screenshots/create_edit_add_tag_delete/before_15');
   cy.wait(2000);

   cy.get('a[class="ember-view gh-back-to-editor"]') // Aumentar el tiempo de espera a 10 segundos
  .contains('Back to dashboard')
  .click();
  cy.screenshot('../screenshots/create_edit_add_tag_delete/before_16');
  cy.wait(500);

    // Seleccioanr el boton de pages.
  cy.get('a[href="#/pages/"]').click();
  cy.screenshot('../screenshots/create_edit_add_tag_delete/before_17');
  cy.wait(500);

   cy.get("div[class='posts-list gh-list  feature-memberAttribution']")
   .contains("Example cypress page")
   .click();
   cy.screenshot('../screenshots/create_edit_add_tag_delete/before_18');
   cy.wait(500);

   cy.get('textarea[class="gh-editor-title  ember-text-area gh-input ember-view"]').clear();
   cy.screenshot('../screenshots/create_edit_add_tag_delete/before_19');
   cy.wait(500);

   cy.get('textarea[class="gh-editor-title  ember-text-area gh-input ember-view"]')
   .type('Edit TAG cypress page');
   cy.screenshot('../screenshots/create_edit_add_tag_delete/before_20');
   cy.wait(1000);

  cy.get('button[title="Settings"]').click();
  cy.screenshot('../screenshots/create_edit_add_tag_delete/before_21');
    cy.wait(500);

  cy.get('#tag-input > ul > input.ember-power-select-trigger-multiple-input')
  .type('News')
  .wait(500);
  cy.screenshot('../screenshots/create_edit_add_tag_delete/before_22');
    cy.wait(500);

  cy.contains('li', 'News')
  .click();
  cy.screenshot('../screenshots/create_edit_add_tag_delete/before_23');
    cy.wait(500);

  cy.get('button[title="Settings"]').click();
  cy.screenshot('../screenshots/create_edit_add_tag_delete/before_24');
  cy.wait(1000);

   cy.get('button.gh-btn.gh-btn-editor.gh-editor-save-trigger.green.ember-view[data-test-button="publish-save"]')
   .contains('Update')
  .click();
  cy.screenshot('../screenshots/create_edit_add_tag_delete/before_25');
  cy.wait(500);

  cy.get('a[href="#/pages/"]').click();
  cy.screenshot('../screenshots/create_edit_add_tag_delete/before_26');
    cy.wait(500);

  cy.get("div[class='posts-list gh-list  feature-memberAttribution']")
  .contains("Edit TAG cypress page")
  .click();
  cy.screenshot('../screenshots/create_edit_add_tag_delete/before_27');
  cy.wait(500);

   cy.get('button[title="Settings"]').click();
   cy.screenshot('../screenshots/create_edit_add_tag_delete/before_28');
   cy.wait(500);

   cy.get('button[data-test-button="delete-post"]').click();
cy.screenshot('../screenshots/create_edit_add_tag_delete/before_29');
    cy.wait(500);

   cy.get('button[data-test-button="delete-post-confirm"]').click();
   cy.screenshot('../screenshots/create_edit_add_tag_delete/before_30');
   cy.wait(500);

   cy.get("div[class='posts-list gh-list  feature-memberAttribution']")
   .should('not.contain',"Edit TAG cypress page");
   cy.screenshot('../screenshots/create_edit_add_tag_delete/before_31');
    cy.wait(500);
})
})
